...
async def get_single_todo(request: Request,
                          todo_id: int = Path(..., title='The ID of todo to retrieve.')):
    for todo in todo_list:
        ...
        raise HTTPException(
            ...
            detail="Todo with the inputed ID does not exist."
        )
        # return {
        #     "message": "Todo with inputed ID does not exist."
        # }
...
async def update_todo(todo_data: TodoItem, todo_id: int = 
    ...
    for todo in todo_list:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Todo with inputed ID does not exist."
        )
        # return {
        #     "message": "Todo with inputed ID does not exist."
        # }
...
async def delete_all_todos() -> dict:
    todo_list.clear()
    return {
            "message": "Todos deleted successfullly."
    }
